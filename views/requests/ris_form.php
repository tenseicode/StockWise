...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0"><?= $isEdit ? 'Edit' : 'New' ?> RIS - Requisition &amp; Issue Slip</h4>
  <a href="<?= $base ?>requests" class="btn btn-outline-secondary"><i class="bi bi-arrow-left"></i> My Requests</a>
</div>

<form method="post" action="<?= $base ?>requests/<?= $isEdit ? 'edit/' . (int)$request['id'] : 'new/RIS' ?>">
  <?= Security::csrfField() ?>
  ...
  <div class="alert alert-info small mt-4 mb-0">
    <i class="bi bi-info-circle"></i> RIS submission requires an <strong>approved PPMP</strong> for your office in the current year. Office limits are applied automatically. Approval chain: Supply Administrator → VP.
  </div>
  <div class="mt-4">
